...
5. axios - это HTTP-клиент, основанный на Promise для node.js и браузера.

## Разработка приложения

Первое, что нужно было сделать — инициализировать Vue.js приложение. Для этого нужно было установить Node.js, в данном случае была задействована версия 17.20.0.
...
После её выполнения, начинается процесс создания проекта. В терминале выводятся следующие вопросы:
1. Какое название будет использоваться?
2. Нужно ли добавлять поддержку TypeSctipt?
3. Нужно ли добавлять vue-ruter?
4. Нужно ли добавлять pinia?
5. Нужно ли добавлять ESLint?
6. Нужно ли добавлять Prettier?

После этого создаётся проект, к директории которого нужно выполнить следующие команды:
...
После этого проект начинает функционировать по адресу http://127.0.0.1:5173

Для редактирования кода использовалась IDE от JetBrains - PhpStorm. Данная IDE позволяет разрабатывать WEB приложения, с использованием различных фреймворков. В нашем случае было разработано Frontend приложение. Разметка была описана с помощью HTML, стилизация производилась с использованием CSS препроцессора SCSS, а за логику работы сайта отвечает TypeScript. 
